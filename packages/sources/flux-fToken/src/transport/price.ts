...
export const httpTransport = new HttpTransport<HttpTransportTypes>({
      ...
      return {
        ...
        request: {
          ...
          params: {
            fToken: param.fToken,
            window: param.window,
            partitionSize: param.partitionSize,
            calculationMethod: param.calculationMethod,
          },
        },
      }
    })
  ...
  parseResponse: (params, response) => {
    ...
    if (!response.data) {
      return params.map((param) => {
        return {
          params: param,
          response: {
            errorMessage: `The data provider didn't return any value for ${param.fToken}} with calculation method ${param.calculationMethod}`,
            statusCode: 502,
          },
        }
      })
    ...
    return params.map((param) => {
      const result = response.data[param.fToken.toUpperCase()].price
